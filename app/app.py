...
def register_node():
    new_node_url = request.json['newNodeUrl']
    node_not_already_present = (i != new_node_url for i in bitcoin.network_nodes)  #채우시오 : new_node_url이 network_noeds에 없으면 true (type boolean)
...
def register_nodes_bulk():
    ...
    for network_node_url in all_network_nodes:
        node_not_already_present = (i != network_node_url for i in bitcoin.network_nodes) #채우시오 : new_node_url이 network_noeds에 없으면 true (type boolean)
